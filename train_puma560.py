...
def jacobian_ik(J_init, T_tgt, max_iter=500, pos_tol=0.1, rot_tol=0.002,
                delta=1.0, lam0=0.01):
    ...
    for _ in range(max_iter):
        ...
        if np.linalg.norm(e_pos) < pos_tol and np.linalg.norm(e_rot) < rot_tol:
            break
        e = np.concatenate([e_pos / R_WORKSPACE, e_rot / math.pi])
        ...
        for k in range(6):
            ...
            Tp = fPUMA(Jp)
            ep = np.concatenate([(T_tgt[:3,3]-Tp[:3,3])/R_WORKSPACE,
                                  rot_vec_err(Tp[:3,:3],T_tgt[:3,:3])/math.pi])
            Jac[:, k] = (ep - e) / delta
        try:
            dJ = np.linalg.solve(Jac.T@Jac + lam*np.eye(6), Jac.T@e)
        ...
        for _ in range(8):
            J_new = J + alpha * dJ
            Tn = fPUMA(J_new)
            en = np.concatenate([(T_tgt[:3,3]-Tn[:3,3])/R_WORKSPACE,
                                  rot_vec_err(Tn[:3,:3],T_tgt[:3,:3])/math.pi])
            if np.linalg.norm(en) < err0: break
            alpha *= 0.5
        J = J + alpha * dJ
        lam = max(lam/3, 1e-7) if np.linalg.norm(en) < err0 else min(lam*5, 1e3)
